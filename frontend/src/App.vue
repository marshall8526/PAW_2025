...
export const routes = [
  ...
  { path: '/tasks', component: TasksList, title: 'Zadania', icon: 'mdi-playlist-check', roles: [0, 1] }
]

export default {
  mixins: [common],
  components: { LoginDialog, LogoutDialog, Dashboard, PersonsList, ProjectsList, ProjectsMap, Analysis, Chat, Admin, TasksList },
  data() {
    return {
      routes,
      operational: false,
...
<template>
  <v-app v-if="operational">
    <v-navigation-drawer permanent density="compact">
      <v-list nav>
        <v-list-item v-for="route in routes" :prepend-icon="route.icon" :title="route.title" :to="route.path"
          v-show="!route.roles || checkIfInRole(session, route.roles)"></v-list-item>
        <v-list-item prepend-icon="mdi-login" title="Login" @click="loginDialog = true"
          v-if="!session.username"></v-list-item>
